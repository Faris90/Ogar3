(function (wHandle, wjQuery) {
	...
	function drawLeaderBoard() {
		...
		if (null != teamScores || 0 != leaderBoard.length)
			if (null != teamScores || showName) {
				...
				lbCanvas.width = 400 * scaleFactor;
				...
				ctx.fillText(c, 200 - ctx.measureText(c).width / 2, 40);
				var b;
				if (null == teamScores) {
					for (ctx.font = "20px Ubuntu", b = 0; b < leaderBoard.length; ++b) {
						...
						if (-1 != nodesOnScreen.indexOf(leaderBoard[b].id)) {
							...
							ctx.fillText(c, 200 - ctx.measureText(c).width / 2, 70 + 24 * b);
						} else {
							ctx.fillStyle = "#FFFFFF";
							if (!noRanking) {
								c = b + 1 + ". " + c;
	...
	var nCanvas, ctx, mainCanvas, lbCanvas, chatCanvas, canvasWidth, canvasHeight, qTree = null,
		...
		ejectIcon = new Image,
		noRanking = false;
	splitIcon.src = "images/split.png";
	ejectIcon.src = "images/feed.png";
