		...
		interval = setInterval(() => {
			FP.then(fp => fp.get()).then(result => {
				...
				if (ban) {
					...
					byId('connecting-content').innerHTML = '<h3>Your are banned 😭</h3><hr class="top" /><p style="text-align: center">You are banned from the game because you broke the rules either spamming the chat or while uploading custom skins.</p><h1 style="text-align: center;">Your unban code is<br /><br />' + btoa(settings.fp).replace(/(.{10})/g, "$1<br />") + '</h1>';
					byId('connecting').show(0.5);
				}
			});
		}, 10000);
