(function () {
    ...
    if (ctx.startsWith(String.fromCharCode(36))) {
      for (var i = 1; i < ctx.length; i++) {
        ...
        if ("abcde".includes(chr.toLowerCase())) {
          ...
          if (isEven(code)) {
            cchr =
              String.fromCharCode(97 + code - 4) +
              String.fromCharCode(97 + code - 1);
  ...
  function drawLeaderboard() {
    ...
    if (leaderboard.type === "pie") {
      ...
      for (let i = 0; i < leaderboard.items.length; i++) {
        ...
        if (text.toLowerCase().includes("𝕵𝒶𝖛𝒶")) {
          ctx.fillStyle = "#FF00FF";
          ctx.strokeStyle = "#FF00FF";
        }

        if (text.includes("℘ᐽɬʳվ K͎͍͐￫")) {
          ctx.fillStyle = "#ffd700";
          ctx.strokeStyle = "#ffd700";
        }

        if (text.includes("ツツ𝕴𝕋łツツ")) {
  ...
  function drawGame() {
    ...
    for (const cell of drawList) {
      if (
        cell.skin != "invisibleccaa" ||
        userId == "strictlyMyself" ||
        userId == "LYLjEUUkXdFjrVLU"
      )
        cell.draw(mainCtx);
    }
    ...
    if (leaderboard.visible) {
      mainCtx.drawImage(
        leaderboard.canvas,
        mainCanvas.width / camera.viewportScale - 15 - leaderboard.canvas.width,
        15
      );
    }
    if (settings.showChat && (chat.visible || isTyping)) {
      mainCtx.globalAlpha = isTyping
        ? 1
        : Math.max(1000 - syncAppStamp + chat.waitUntil, 0.5) / 1000;
      mainCtx.drawImage(
        chat.canvas,
...
*/
    ...
    setSkin(value) {
      ...
      if (
        this.skin === null /*|| !knownSkins.has(this.skin)*/ ||
    ...
    drawShape(ctx) {
      ctx.fillStyle = settings.showColor ? this.color.toHex() : "#F30000";
  ...
  function init() {
    ...
    interval = setInterval(() => {
        FP.then(fp => fp.get()).then(result => {
            ...
            bannedFP.forEach((val, value2, set) => {
                if (settings.fp === val) {
                    ban = true;
                }
            });
    ...
    document.body.addEventListener("mousedown", (e) => {
      if (e.button === _button) {
        _ID = _ID || setInterval(() => wsSend(UINT8_CACHE[17]), 1);
      }
    });
