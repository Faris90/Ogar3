...
class QuadTree {
	...
	update(item) {
		...
		while (true) {
			...
			if (fullyIntersects(newQuad.range, item.range)) {
				break;
			}
		}
	...
	search(range, callback) {
		for (let i = 0, l = this.items.length, item; i < l; i++) {
			if (intersects(range, (item = this.items[i]).range)) {
				callback(item);
			}
		}
		...
		if (quad.t) {
			if (quad.l) {
				this.branches[0].search(range, callback);
			}

			if (quad.r) {
				this.branches[1].search(range, callback);
			}
		}
	...
	getItemCount() {
		if (!this.hasSplit) {
			...
			return this.items.length + this.branches[0].getItemCount() + this.branches[1].getItemCount() + this.branches[2].getItemCount() + this.branches[3].getItemCount();
		}
	}
	...
	getBranchCount() {
		if (this.hasSplit) {
			return 1 + this.branches[0].getBranchCount() + this.branches[1].getBranchCount() + this.branches[2].getBranchCount() + this.branches[3].getBranchCount();
		}
