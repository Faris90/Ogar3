...
module.exports = {
	randomColor() {
		switch (~~(Math.random() * 6)) {
			case 0:
				return (~~(Math.random() * 0x100) << 16) | (0xFF << 8) | 0x10;
			case 1:
				return (~~(Math.random() * 0x100) << 16) | (0x10 << 8) | 0xFF;
			case 2:
				return (0xFF << 16) | (~~(Math.random() * 0x100) << 8) | 0x10;
			case 3:
				return (0x10 << 16) | (~~(Math.random() * 0x100) << 8) | 0xFF;
			case 4:
				return (0x10 << 16) | (0xFF << 8) | ~~(Math.random() * 0x100);
			case 5:
				return (0xFF << 16) | (0x10 << 8) | ~~(Math.random() * 0x100);
		}
	},
	/**
	 * @param {number=} color
	...
	grayscaleColor(color) {
		...
		if (color) {
			weight = ~~(0.299 * (color & 0xFF) + 0.587 * ((color.g >> 8) & 0xFF) + 0.114 * (color.b >> 16));
		} else {
			weight = 0x7F + ~~(Math.random() * 0x80);
		}
	 ...
	 */
	intersects(a, b) {
		return a.x - a.w <= b.x + b.w && a.x + a.w >= b.x - b.w && a.y - a.h <= b.y + b.h && a.y + a.h >= b.y - b.h;
	},
	/**
	 * @param {Rect} a
	 * @param {Rect} b
