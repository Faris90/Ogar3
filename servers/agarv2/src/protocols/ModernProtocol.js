...
class ModernProtocol extends Protocol {
	...
	get subtype() {
		return `m${!isNaN(this.protocol) ? ("00" + this.protocol).slice(-2) : "//"}`;
	}
	...
	distinguishes(reader) {
		...
		if (this.protocol !== 3) {
			return void this.fail(1003, "Unsupported protocol version");
		}
	...
	onVisibleCellUpdate(add, upd, eat, del) {
		...
		if (this.clearCellsPending) {
			globalFlags |= 64, this.clearCellsPending = false;
		}
		...
		if ((l = add.length) > 0) {
			for (i = 0; i < l; i++) {
				...
				if (!!item.skin) {
					writer.writeZTStringUTF8(item.skin);
				}
			}
