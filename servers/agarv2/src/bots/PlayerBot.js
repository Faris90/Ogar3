...
class PlayerBot extends Bot {
	...
	update() {
		if (this.splitCooldownTicks > 0) {
			this.splitCooldownTicks--;
		} else {
			this.target = null;
		}
		...
		for (let id in player.visibleCells) {
			...
			switch (check.type) {
				...
				case 3:
					...
					break;
				case 4:
					if (this.canEat(check.size, cell.size)) {
						influence = -1;
					} else if (this.canEat(cell.size, check.size)) {
						if (atMaxCells) {
							influence = truncatedInfluence * cellCount;
		...
		if (willingToSplit && !splitkillObstacleNearby && this.splitCooldownTicks <= 0 && bestPrey !== null && bestPrey.size * 2 > cell.size) {
	...
	canSplitkill(aSize, bSize, d) {
		const splitDistance = Math.max(2 * aSize / this.listener.settings.playerSplitSizeDiv / 2, this.listener.settings.playerSplitBoost);
		return aSize / this.listener.settings.playerSplitSizeDiv > bSize * this.listener.settings.worldEatMult && d - splitDistance <= aSize - bSize / this.listener.settings.worldEatOverlapDiv;
	}
}
