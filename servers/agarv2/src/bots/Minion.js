...
class Minion extends Bot {
	...
	get shouldClose() {
		return !this.hasPlayer || !this.player.exists || !this.player.hasWorld || this.following.socketDisconnected || this.following.disconnected || !this.following.hasPlayer || !this.following.player.exists || this.following.player.world !== this.player.world;
	}

	update() {
		const name = this.listener.settings.minionName === "*" ? `*${this.following.player.leaderboardName}` : this.listener.settings.minionName;
